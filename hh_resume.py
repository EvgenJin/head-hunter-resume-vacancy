...
def if_null_num(par):
  if par:
    input = re.sub(r"\s+", "", par.get_text(), flags=re.UNICODE)
    res = re.match(r'\d+',input).group(0)
    res = int(res)
  else:
    res = None
  return res
...
def get_resume(query,area,period):
  ...
  while len(get_all(query,area,period,i)) > 0:
    ...
    for item in result_all:
      ...
      row = [if_null_bf(title),dbirth,if_null_num(salary),if_null_bf(last_work),if_null_bf(last_work_per),'https://hh.ru'+id]
      rows.append(row)

  df = pd.DataFrame(rows, columns=columns)
  # print(df[['title','dbirth','salary','last_work']])
  # return(df.agg({'salary':['mean','median','count']}))
  return df
